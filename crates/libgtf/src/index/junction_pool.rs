...
impl JunctionPool {
    ...
    pub fn add(&mut self, junctions: &[u32]) -> Result<JunctionSpan, IndexDataError> {
        Self::validate_junctions(junctions)?;

        let offset =
            u32::try_from(self.junctions.len()).map_err(|_| IndexDataError::PoolTooLarge)?;
        let count =
            u16::try_from(junctions.len()).map_err(|_| IndexDataError::TooManyJunctions {
                count: junctions.len(),
            })?;
    ...
    pub fn get(&self, span: JunctionSpan) -> Result<&[u32], IndexDataError> {
        let start: usize =
            usize::try_from(span.offset).map_err(|_| IndexDataError::InvalidSpan {
                offset: span.offset,
                count: span.count,
    ...
    fn validate_junctions(junctions: &[u32]) -> Result<(), IndexDataError> {
        if !junctions.len().is_multiple_of(2) {
            return Err(IndexDataError::InvalidEncoding {
                msg: format!(
        ...
        {
            return Err(IndexDataError::JunctionsNotStrictlyIncreasing);
        }
        Ok(())
    }
}
