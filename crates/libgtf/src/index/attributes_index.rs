...
use crate::{error::Error, index::ISOMS_VERSION};
...
impl AttrIndexReader {
    fn read_header(file: &mut File) -> Result<AttrIndexHeader, Error> {
        let mut magic = [0u8; 5];
        ...
        if version != ISOMS_VERSION {
            return Err(Error::UnsupportedVersion {
                found: version,
                expected: ISOMS_VERSION,
            });
        }

        let mut md5 = [0u8; 16];
        file.read_exact(&mut md5).map_err(|e| Error::InvalidIndex {
            reason: format!("Can not read version in AttrIndex file: {}", e),
        })?;
    ...
    pub fn load_header<P: AsRef<Path>>(path: P) -> Result<AttrIndexHeader, Error> {
    ...
    pub fn get_attr(&mut self, tx_idx: u64) -> Result<Option<Vec<u8>>, Error> {
        if tx_idx >= self.header.total_tx_n {
            return Ok(None);
        }
        let e: fn(std::io::Error) -> Error = |err| Error::InvalidIndex {
        ...
        let data = zstd::decode_all(compressed.as_slice()).map_err(|err| Error::InvalidIndex {
            reason: err.to_string(),
        })?;
...
impl AttrIndexBuilder {
    pub fn new(file: File, total_tx_n: u64, md5: &[u8; 16]) -> Result<Self, Error> {
    ...
    pub fn dump_attr(&mut self, data: Vec<u8>, tx_gidx: u64) -> Result<usize, Error> {
        let idx = usize::try_from(tx_gidx).map_err(|_| Error::InvalidIndex {
        ...
        let compressed = zstd::encode_all(data.as_slice(), 3).map_err(|e| Error::InvalidIndex {
            reason: e.to_string(),
        })?;
        let offset = u64::try_from(self.blob_offset).map_err(|_| Error::InvalidIndex {
            reason: format!("blob offset {} exceeded u64", self.blob_offset),
        })?;
        let length = u32::try_from(compressed.len()).map_err(|_| Error::InvalidIndex {
