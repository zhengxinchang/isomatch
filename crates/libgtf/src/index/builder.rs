...
use crate::index::TxBase;
use crate::index::{ChromBlockBuilder, ChromDirectoryEntry, IndexHeader};
...
fn compress(raw: Vec<u8>) -> std::io::Result<Vec<u8>> {
    zstd::encode_all(raw.as_slice(), 3).map_err(std::io::Error::other)
}
...
fn encode_compressed<T>(pool: &T, writer: &mut impl Write) -> error::Result<usize>
where
    T: Encodable<Error = IndexDataError>,
{
    let mut raw = Vec::new();
...
impl IndexBuilder {
    ...
    pub fn add_chrom(&mut self, entry: ChromBlockBuilder) -> error::Result<()> {
        // entry.finalize();
