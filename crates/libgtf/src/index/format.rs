...
use crate::{
    error::{self, FastaError},
    fasta::FastaReader,
    gtf::{Strand, Transcript},
    index::{
        ISOMX_VERSION, JunctionPool, SpliceSitePair, SpliceSitePool, SpliceSiteSpan, StringPool,
        TxBase,
    },
    traits::{Decodable, DiskSize, Encodable},
    utils::hash_u8_slice,
};
...
pub struct TxSummary {
    ...
    pub canonical_junction_count: usize,
}
...
impl ChromBlockBuilder {
    ...
    pub fn add_tx(
        ...
        let junction_span = self.junction_pool.add(&intron)?;

        let tx_id_span = self.string_pool.add(gtf_tx.tx_id.as_str())?;

        let gene_id_span = self.string_pool.add(gtf_tx.gene_id.as_str())?;

        let (refhash, splice_site_pairs) = if gtf_tx.exons.len() == 1 {
            // if mono-exon, dont need to do this calculation
            (0, Vec::new())
        } else {
            let reference_seq = refr.fetch(
                chrom_name,
            ...
            let left_exon = exon_offsets
                .first_mut()
                .ok_or_else(|| FastaError::FetchSeqFailed {
            ...
            // refhash
            (hash_u8_slice(&tx_sequence), splice_site_pairs)
        };
            ...
            Some(reader) => {
                if gtf_tx.exons.len() == 1 {
                    ...
                    if sequence.len() != tx_seq_len {
                        dbg!(&gtf_tx);
                        return Err(error::FastaError::FetchSeqFailed {
                    ...
                    let first_exon = gtf_tx.exons.first().ok_or(FastaError::FetchSeqFailed {
                        reason: "No exons found".to_string(),
                    })?;

                    let last_exon = gtf_tx.exons.last().ok_or(FastaError::FetchSeqFailed {
                        reason: "No exons found".to_string(),
                    })?;
                    ...
                    let left_trim = tss_exon_len.saturating_sub(3);
                    ...
                    hash_u8_slice(sliced_seq)
                }
            }
            None => 0u128,
        };

        let canonical_junction_count = splice_site_pairs.iter().try_fold(
            0usize,
            |count, (left, right)| -> error::Result<usize> {
                let pair = SpliceSitePair::pack(left, right, gtf_tx.strand)?;
                Ok(count + usize::from(pair.is_canonical()))
            },
        )?;
        ...
        let splice_site_span = if splice_site_pairs.is_empty() {
            ...
            self.splice_site_pool
                ...
                .map_err(|e| Error::BuildTranscript {
                    tx_id: gtf_tx.tx_id.clone(),
                    reason: e.to_string(),
                })?
        };
        ...
        Ok(TxSummary {
            strand: gtf_tx.strand,
            exon_count: gtf_tx.exons.len(),
